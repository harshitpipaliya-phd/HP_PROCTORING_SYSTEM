...
def insert_log(looking_away: bool, head_direction: str, attention_score: int):
    """Legacy proctoring log insert.
    
    FIX: 'proctoring_logs' table does not exist in any migration.
         Redirected to 'proctoring_logs' which must be created via Fix C-2 SQL,
         OR change to 'behavior_logs' if you prefer not to create the extra table.
    """
    try:
        if not _db_available or _supabase is None:
            return None
        ...
        # FIX: 'proctoring_logs' table created by Fix C-2 SQL
        # Alternatively change to "behavior_logs" to avoid creating another table
        return _supabase.table("proctoring_logs").insert(data).execute()
    except Exception as e:
        print(f"[DB ERROR insert_log]: {e}")
        return None
...
def fetch_sessions(user_id: str = None, limit: int = 20) -> List[Dict[str, Any]]:
    ...
    FIX: Was querying legacy 'sessions' table; changed to 'proctoring_sessions'.
         ORDER BY changed from 'timestamp' to 'created_at' (correct column name).
    """
    try:
        if not _db_available or _supabase is None:
            return []
        
        # FIX: use proctoring_sessions (real table), order by created_at
        query = _supabase.table("proctoring_sessions").select("*").order("created_at", desc=True).limit(limit)
...
def fetch_reports(session_id: str = None, limit: int = 20) -> List[Dict[str, Any]]:
    ...
    FIX: ORDER BY changed from 'timestamp' to 'created_at'.
         The 'reports' table has no 'timestamp' column.
    """
    try:
        if not _db_available or _supabase is None:
            return []
        
        # FIX: 'reports' table has 'created_at', not 'timestamp'
        query = _supabase.table("reports").select("*").order("created_at", desc=True).limit(limit)
...
def fetch_events_for_session(session_id: str, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # FIX: 'events' table has 'created_at', not 'timestamp'
        query = (
            _supabase.table("events")
            .select("*")
            .eq("session_id", session_id)
